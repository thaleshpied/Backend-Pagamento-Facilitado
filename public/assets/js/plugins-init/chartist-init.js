(function($) {
 ...
 var dlabChartlist = function(){
	...
	// GRÁFICO TOP 10 OBRAS COM MAIOR NÚMERO DE REVISÕES 
	var top10revisoes = function(){
		//Horizontal bar chart
  
			let labels = [];
			let series = [];
		
			// Seleciona todos os inputs que armazenam os nomes e valores
			document.querySelectorAll('input[id^="centrodecustorevisao"]').forEach(input => {
				const ref = input.id.replace("centrodecustorevisao", ""); // Obtém o número do índice
				const nome = input.value; // Nome da obra / centro de custo
				const revisaoInput = document.getElementById(`totalrevisoes${ref}`); // Encontra o total de revisões correspondente
		
				if (revisaoInput) {
					const revisoes = parseInt(revisaoInput.value) || 0;
					labels.push(nome);
					series.push(revisoes);
				}
			});
			...
			// Renderiza o gráfico Chartist com os dados capturados
			new Chartist.Bar('#top10revisoes', {
				labels: labels,
				series: [series]
			}, {
				...
				axisX: {
					showLabel: true // Mostra os valores no eixo X
				}
			}).on('draw', function (data) {
				if (data.type === 'bar') {
					data.group.append(
						new Chartist.Svg('text', {
							...
							y: data.y2,
							'text-anchor': 'start',
							style: 'font-size: 18px; fill: black;'
						}, 'ct-label').text(data.value.x)
					);
				}
			});
	...
	var top10previstoxrealizado = function () {
		new Chartist.Bar('#top10previstoxrealizado', {
			labels: ['NOME OBRA', 'NOME OBRA', 'NOME OBRA', 'NOME OBRA', 'NOME OBRA', 'NOME OBRA', 'NOME OBRA', 'NOME OBRA', 'NOME OBRA', 'NOME OBRA'],
			series: [
			...
			axisX: {
				showLabel: false // Esconde os valores da escala no rodapé
			}
		}).on('draw', function (data) {
			if (data.type === 'bar') {
				data.group.append(
	...
	var top10Lancamentos = function(){
		//Horizontal bar chart
			...
			// Seleciona todos os inputs que armazenam os nomes e valores
			document.querySelectorAll('input[id^="centrodecustolancamento"]').forEach(input => {
				const ref = input.id.replace("centrodecustolancamento", ""); // Obtém o número do índice
				const nome = input.value; // Nome da obra / centro de custo
				const revisaoInput = document.getElementById(`totallancamentos${ref}`); // Encontra o total de revisões correspondente
			...
			// Renderiza o gráfico Chartist com os dados capturados
			new Chartist.Bar('#top10Lancamentos', {
				labels: labels,
				series: [series]
			}, {
				seriesBarDistance: 80,
	...
	// 			const ref = input.id.replace("centrodecustolancamento", ""); // Obtém o número do índice
	// 			const nome = input.value; // Nome da obra / centro de custo
	// 			const revisaoInput = document.getElementById(`totallancamentossegunda${ref}`); // Encontra o total de revisões correspondente
